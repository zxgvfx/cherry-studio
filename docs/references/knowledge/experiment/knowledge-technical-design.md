...
The v2 goal: align the knowledge base's underlying data shape with the future folder-backed design — one engine-portable `KnowledgeBase/{baseId}/.cherry/index.sqlite` per base (7-table material model), so the v2 → v2.x switch only moves/reuses the index. The global `knowledge_base` / `knowledge_item` tables stay **permanently** — `knowledge_item` is the business-state authority that drives the UI (no "exit path"; see §7), and the per-base folder is internal, Cherry-managed byte storage. **Update (2026-07-01)**: embedding is optional — a base created without one is BM25-only (lexical search), landing in v2 directly rather than the originally-scoped v2.x degradation; vector/hybrid retrieval still needs an embedding model.
...
interface KnowledgeIndexStore {
  ...
  deleteMaterials(materialIds: string[]): Promise<void> // async only to yield between batched deletes
...
`pipeline/indexing/embed.ts` → `AiService.embedMany`, `pipeline/indexing/rerank.ts` → `AiService.rerank`, reusing the provider the user configured on the chat side (`provider::model` UniqueModelId). No local ONNX inference stack. Persistent rerank misconfiguration (401/403/404) escalates to an error log; transient failures fall back to the un-reranked results.
...
`KnowledgeIndexStore.search()` is the **single retrieval entry point** for both lanes: BM25 (`search_text_fts`) / vector (`embedding`) / hybrid (RRF fusion — rank-based, so the two incompatible score scales need no normalization). Results come from `search_unit` (its `material_id` FK guarantees a live material, so no material-level filter is applied); the caller filters `knowledge_item.status = 'completed'`. **Update (2026-07-01)**: vector-less degradation (BM25-only) is supported — a base with no embedding model runs lexical-only search instead of erroring.

### 6.1 search() wiring and retrieval tuning

`documentCount` and `threshold` are the base-level retrieval tuning columns left; `search()` reads `documentCount` from the base row as the result cap (`documentCount ?? 10`) and applies `threshold` as a relevance cutoff on scored hits (`applyRelevanceThreshold` in `query/search.ts` — it filters vector-mode or post-rerank relevance scores and is a no-op for BM25/RRF ranking scores). Retrieval mode is derived at runtime: bases with a completed vector config use hybrid retrieval, and bases without an embedding model fall back to BM25.
...
- **PR C (v2.x)**: material-level results + locator/read, editable index entries (with their `content_index_entry` table), kb__read / kb__tree / kb__manage tool surface, per-result score semantics. (BM25-only degradation, originally scoped here, landed in v2 directly instead — see §1 and §6.)
- **v1 directory migration — re-attribute vectors, tombstone as fallback (decided 2026-06-11 as tombstone-only, reversed to re-attribution 2026-06-13)**: a v1-indexed folder migrates by re-using its already-embedded vectors instead of dropping them. `KnowledgeMigrator` expands the `directory` item into a `completed` container `directory` plus one `completed` `file` child per embedded file (`expandLegacyDirectoryItem`), reading each file's path from the legacy `vectors.source` column (`loadLoaderSourceMap`) and publishing a loader-id → child-item-id remap over `sharedData`; `KnowledgeVectorMigrator` consumes that remap so the folder's chunks land on the per-file children rather than being skipped as `non_indexable_container`. The v1 vectors are reused verbatim — no re-embedding. Paths mirror a native directory expansion exactly: the container claims a deduped top-level `raw/` prefix (`docs`, `docs_1`, … — the same scheme as `chooseDirectoryPathPrefix`) and each child carries the external `source` plus `<prefix>/<path relative to the folder>`, so display names, Concept IDs and `material.relative_path` read as real paths instead of opaque ids. The subtree path is derived **purely from the v1 `source` strings** (both separators, case-folded comparison, per-segment `sanitizeFilename`) with no filesystem access, so the same v1 export migrates identically on macOS and Windows. Prefixes are deduped per base — during migration against a set `KnowledgeMigrator.prepare` carries (seeded with the reserved `.cherry`), at runtime through `collectReservedTopLevelNames` — and the migration's file-copy pass seeds those prefixes into its reserved set so a copied file can never claim a folder's namespace (deleting or re-indexing that folder `removeDir`s the whole prefix). Crucially, **the paths are shaped like real ones but backed by no bytes**: the originals are **not** copied into the base (v1 never stored the folder inside Cherry) and the v1 `source` path is untrustworthy, so search uses the migrated vectors directly, there is no `raw/` file, and the child is never read from disk. If the original folder is still on disk, re-indexing the container fills `raw/<prefix>` for real and the migrated shape converges on the native one; the prefix is normally re-picked identically (`chooseDirectoryPathPrefix` excludes the container itself and derives from the same basename), so paths and display names do not churn — unless the prefix took a `_N` at migration time and whatever forced it has since been deleted, in which case the reindex reclaims the shorter name. Synthesized children are inserted after their container (FK `(baseId, groupId) → (baseId, id)`) and are skipped by the file-copy pass (they have no storage name to back-fill). **Fallback:** when the legacy vectors are unreadable/empty or the folder has no loader ids, `expandLegacyDirectoryItem` returns null and the item keeps the original tombstone — `failed` + `directory_not_migrated` (UI: red failure asking the user to delete and re-upload the folder); because the tombstone now appears only when vectors genuinely could not migrate, the message is accurate (it no longer fires for folders whose data did migrate). Background: v1 embedded each folder file individually but booked every loader id on the one directory item (no per-file items), so without this re-attribution the vectors are dropped as `non_indexable_container` and the v2 index comes up empty. **Known v2.0 limitation (MVP):** a migrated child has no `raw/` file and an untrustworthy v1 `source`, so it cannot be rebuilt from disk. Reindex deletes a subtree's vectors before re-reading its source, so `KnowledgeIngestionService.assertSubtreesCanReindex` first checks the reindex root's source still exists — a directory's original folder (`data.source`) or a file leaf's `raw/` file (`indexedRelativePath ?? relativePath`) — and **rejects the reindex up front** when it is gone, so the migrated vectors are never destroyed. This is a general missing-source guard, not a migration-specific flag: a normal item whose source file was deleted is rejected the same way. Rebuilding a migrated folder means deleting it and re-adding it as a fresh directory import — and if its original folder is still on disk, reindexing the container rebuilds it for real (rescanned from `data.source`). The earlier-rejected placeholder-file alternative stays rejected (synthetic content would bake in as truth on later reindex; binary extensions cannot host text). The legacy embedjs DB stays in place (rollback insurance).
- **Watcher preconditions (surfaced by the directory-migration review)** — constraints for the future file-watcher PR:
  - Index cleanup must be driven by user intent (explicit delete/reindex), never inferred from a missing file: url/note materials use the item id as a virtual `relative_path` with no on-disk file, migrated v1 directory children carry a **path-shaped but unbacked** `relative_path` (`<prefix>/<subpath>` derived from the v1 source; their `source` points at the file's own original external path — the folder path lives on the container — never copied into the base), and migrated urls have no snapshot until their first reindex. (URL materialization landed in PR B, see §1; notes keep a virtual path and migrated directory children keep an unbacked one.) The directory children make this rule *more* important, not less: their paths now look exactly like real ones, so "the file is not there" says nothing about user intent.
  - A filesystem move/rename arrives as unlink+add; naive handling would `deleteMaterials` (whose in-transaction GC immediately drops the shared embeddings) and then re-embed the "new" file at full cost, defeating A4 reuse. Needs rename detection, event debounce/merge, or a GC grace period.
  - An external source folder disappearing (`data.source`) is not a delete intent: mark the source unavailable; never clean the base copy or its index.
